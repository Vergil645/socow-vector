...
struct socow_vector {
    ...
    socow_vector() noexcept : _stat_buffer() {}
    ...
    void swap(socow_vector& other) {
        ...
        if (_is_small) {
            std::swap(_stat_buffer, other._stat_buffer);
        } else {
...
private:
    struct dynamic_storage {
        dynamic_storage() noexcept : _data(nullptr) {}
        ...
        if (_is_small) {
            if (new_cap <= SMALL_SIZE) {
                ...
                for (size_t i = 0; i != _size; i++) {
                    try {
                        new (new_dyn_buffer->_data + i) T(operator[](i));
                    } catch (...) {
            ...
            if (new_cap <= SMALL_SIZE) {
                ...
                for (size_t i = 0; i != _size; i++) {
                    try {
                        new (&new_stat_buffer[i]) T(operator[](i));
                    } catch (...) {
                        clear_prefix(new_stat_buffer, i);
                        new_stat_buffer.~array();
                        throw;
                    }
                }
                if (_dyn_buffer->_copy_count == 1) {
